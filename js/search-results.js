 ...
 * Features: Search results display, filtering, pagination
 */
...
function initializePage() {
    if (typeof SEARCH_QUERY !== 'undefined' && SEARCH_QUERY) {
        performSearch(SEARCH_QUERY, 1);
        loadFilters();
        
        // Set initial filter values
        if (typeof CATEGORY_FILTER !== 'undefined' && CATEGORY_FILTER > 0) {
            $('#categoryFilter').val(CATEGORY_FILTER);
        }
        if (typeof BRAND_FILTER !== 'undefined' && BRAND_FILTER > 0) {
            $('#brandFilter').val(BRAND_FILTER);
        }
    } else {
...
function initializePagination() {
    ...
    $(document).on('click', '.page-link', function(e) {
        ...
        if (page) {
            performSearch(SEARCH_QUERY, page);
        }
    });
}
...
function initializeProductInteractions() {
    ...
    $(document).on('mouseenter', '.product-card-search', function() {
        $(this).find('.product-image-search').addClass('hover-effect');
    }).on('mouseleave', '.product-card-search', function() {
        $(this).find('.product-image-search').removeClass('hover-effect');
    });
}
 ...
 * Perform search
 */
function performSearch(searchQuery, page = 1, filters = {}) {
    showLoading();
...
function applyFilters() {
    const filters = {
        ...
        sort: $('#sortFilter').val() || 'relevance'
    };
    
    performSearch(SEARCH_QUERY, 1, filters);
}
...
function displaySearchResults(products) {
            ...
            <div class="col-12">
                <div class="no-results text-center py-5">
                    ...
                    <div class="mt-4">
                        ...
                        <button class="btn btn-primary" onclick="window.location.href='product.php'">
                            <i class="fa fa-search me-2"></i>Try New Search
                        </button>
                    </div>
                </div>
            </div>
    ...
    products.forEach(function(product) {
        ...
        const highlightedTitle = highlightSearchTerms(product.product_title, SEARCH_QUERY);
        
        html += `
            <div class="col-md-6 col-lg-4 mb-4">
                <div class="card product-card-search h-100" data-product-id="${product.product_id}">
                    <div class="product-image-container">
                        <img src="${imageSrc}" class="product-image-search" alt="${product.product_title}" 
 ...
 * Update results info
 */
function updateResultsInfo(total, page, searchQuery) {
    $('#resultsCount').text(total);
    $('#currentPage').text(page);
    $('#currentSearchQuery').text(searchQuery);
    $('#resultsInfo').show();
}
